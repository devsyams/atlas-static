 ...
 * BFF (A7) and the BUMN-board aggregation BFF (A7 v37). Holds the env key and
 * the Next data-cache options in one place so the secret + caching live once.
 * No date params are sent — the upstream applies its own default 7-day window
 * (v42.0/v5.0). Server-only (reads `process.env`, calls fetch).
 */
...
async function fetchWindow(
  ...
  const url = buildTopicsUrl(base, code, apiKey);
...
/**
 * Fetch + map one topic code over the upstream's default window. `fresh`
 * bypasses the data cache. Throws `FeedNotConfiguredError` if no key, or a
...
export async function fetchTopicsForCode(
  ...
  if (!fresh && result.issues.length === 0) {
    try {
      const live = await fetchWindow(base, code, apiKey, true);
