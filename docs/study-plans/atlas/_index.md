...
| 1.83 | 2026-06-15 | P8 → v3.0 Built (MAJOR, TDD) — topic deep link resolves: mint through **OpenGate** `autologin_generate` with `redirect` baked into the generate call, 307 straight to `login_url`; lands topic-precise on garudaperkasa `dashboard_demo?id=monitoring&idquery=…`. Route repointed to `OPENGATE_*`; `NEXORUS_DASHBOARD_AUTOLOGIN_BASE`/`_API_KEY` retired. AC7/AC9 amended, T7/T8 reworked; backend ask marked resolved (1.82 reserved by the parallel A7 v44.0 branch) |
