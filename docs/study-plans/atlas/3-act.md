...
- **Version:** 5.0 · **Stage:** 3-act · **Sprint:** demo · **Status:** Built · **Spec ref:** built on A7's Danantara topics feed + the OpenGate `/threats` feed; CEO feedback 2026-06-22 · **Owner:** Dev A
...
- **AC8** *(v5.0 — real `/threats` wiring)* — *Given* the live OpenGate `/threats` feed (`/api/v1/danantara/threats` → the #1 detected threat + its drivers), *When* the gate loads, *Then* the **middle** column names that **detected threat** (title + severity `n/10` + growth-rate + impact line + **trending keywords** as "topik pendorong"), and the **right** column lists its **real driving accounts** — mapped from `top_impact_posts[].actor_intelligence` (`@handle`, platform, followers, credibility, risk), split **human (`real_person`) vs provocateur/bot** — deduped by handle, ranked by engagement. The **left** dial is unchanged (`/topics`). *When* the feed has no detected threat (calm period) or is unavailable, both columns degrade to their neutral empty states (no crash), per AC5.
