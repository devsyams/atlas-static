...
- **Version:** 2.0 · **Stage:** 3-act · **Sprint:** demo · **Status:** In progress · **Spec ref:** `docs/superpowers/specs/2026-08-01-board-level-threat-index-response-simulator.md` · **Owner:** Dev A
...
- **AC9** *(v2.0)* — *Given* the board simulator, *When* it renders, *Then* it shows a compact **Threat Index headline** (`now → modeled post-response`, computed at the default Professional tier) above **three tier cards**. Each card shows the tier name (colour-coded), its noise multiplier (`×N NOISE`), a strategy tagline, the **counter-actions** total and the **Clipper / Homeless / KOL** split; **Professional** carries a `DEFAULT` badge. The prior single-tier metric grid (recommended volume, projected reach, negative-volume baseline, grassroots) is **removed**.
...
| T27 | AC9 | *(v2.0)* the simulator renders the **Threat Index headline** (now → post-response) + **three tier cards** (Enterprise/Professional/Basic), each with its counter-actions and Clipper/Homeless/KOL split; Professional shows `DEFAULT`; the `×5/×3/×1 NOISE` pills are present | component |
