...
- **Version:** 41.3 · **Stage:** 3-act · **Sprint:** demo · **Status:** Built · **Spec ref:** `docs/superpowers/specs/2026-06-02-danantara-ceo-command-design.md` · **Owner:** Dev A
...
**Acceptance criteria:**
- **AC1** *(amended v3.7)* — *Given* a registered BUMN slug, *When* `/bumn/‹slug›` loads, *Then* it renders that BUMN's dashboard driven by a **registry** (`slug → name → topicCode`), with a **header** showing the **BUMN logo on the left** of the eyebrow / **BUMN name** / subtitle stack *(v3.7 — real `/bumn/‹slug›.png` via the shared `BumnLogo`, monogram fallback)*; *And When* the slug is not in the registry, *Then* it 404s. A super-admin-only **`/bumn` index** lists all registered BUMN with links.
...
- **AC6** — *Given* a BUMN with **no topics** in the window even after widening to 28 days (e.g. Mandiri, BRI), *When* the dashboard renders, *Then* it still shows the Intent Share leaderboard (and the Sentiment Summary when `summary` is present) plus a clear **"No topics in this window"** state — the page never blanks.
...
### A9. Counter-Noise — Response Calculator

- **Version:** 3.0 · **Stage:** 3-act · **Sprint:** demo · **Status:** Built · **Spec ref:** built on A7's Danantara topic detail · **Owner:** Dev A
