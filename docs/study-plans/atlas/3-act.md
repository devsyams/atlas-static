...
- **AC14** *(v2.0)* — *Given* the volume anchor (the board's negative-post baseline), *When* a tier card is computed, *Then* its **counter-actions = anchor × tier multiplier** (Basic ×1, Professional ×3, Enterprise ×5), split **Clipper 50% / Homeless 20% / KOL 30%** — pure, deterministic and identical to A9's `responseCalculator`, so the three cards read as one honest comparison of the same baseline at three intensities.
...
- *(v2.0)* `change` `lib/danantara/ceo/counter-noise.ts` — add `TIER_STRATEGY` (the per-tier taglines) beside `TIER_LABEL` / `TIER_MULTIPLIER`.
...
| T36 | AC14 | `responseCalculator(anchor, tier)`: counter-actions = anchor × {1,3,5}; split Clipper 50% / Homeless 20% / KOL 30%; the three per-tier values are internally consistent | pure |
