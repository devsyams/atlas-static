...
- **Version:** 43.0 · **Stage:** 3-act · **Sprint:** demo · **Status:** Built · **Spec ref:** `docs/superpowers/specs/2026-06-02-danantara-ceo-command-design.md` · **Owner:** Dev A
...
- **AC3** *(amended v4.0, v7.0, v11.0, v37.0)* — *Given* the BUMN board, *When* rendered, *Then* the **7 launch BUMN** *(v37.0: was 20 mock; now the registry set — Mandiri, PLN, Telkom, Pertamina, BNI, BRI, Jasa Marga)* render as a **single full-width list, one BUMN per row** (most-negative first), each row prefixed with a **sequential rank number** plus its rank badge, with net sentiment encoded as a **green↔red row tint**. *(v7.0: replaces the grouped heatmap tiles; see AC16/AC18.)*
...
- **Version:** 7.0 · **Stage:** 3-act · **Sprint:** demo · **Status:** Built · **Spec ref:** built on A7's live topics feed (`docs/superpowers/specs/2026-06-02-danantara-ceo-command-design.md`) · **Owner:** Dev A
...
**v7.0 — roster expansion (2026-06-14, client):** the portfolio to be listed grew from 7 to **36 BUMN** — the client supplied the full topic-code list, all verified live (HTTP 200; 34 with data, 2 temporarily empty — `jamkrindosyariah`, `asdpindonesia` — which the AC6 empty state + AC2 widening already cover). The 29 new codes span financial (BSI, ASABRI, Askrindo(+Syariah), Jamkrindo(+Syariah), Pegadaian, Manajemen Aset), construction/transport (Adhi/Hutama/PP/Waskita/Wijaya Karya, Whoosh, Pelindo, ASDP, PELNI), industri & logistik (Pos, Garuda, Citilink, Pelita Air, Galangan Kapal, Kimia Farma, RSPP, Hotel Indonesia Natour, InJourney, Wisata Borobudur), and pangan/konsumer (AgriNas Palma, Sarinah). Sectors are best-effort mapped to the existing 7-key taxonomy (no new keys); new BUMN have no logo asset yet and use the monogram fallback.

**Acceptance criteria:**
- **AC1** *(amended v3.7, v7.0)* — *Given* a registered BUMN slug, *When* `/bumn/‹slug›` loads, *Then* it renders that BUMN's dashboard driven by a **registry** (`slug → name → short → sector → topicCode`, with `topicCode === danantara_‹slug›`), with a **header** showing the **BUMN logo on the left** of the eyebrow / **BUMN name** / subtitle stack *(v3.7 — real `/bumn/‹slug›.png` via the shared `BumnLogo`, monogram fallback)*; *And When* the slug is not in the registry, *Then* it 404s. A super-admin-only **`/bumn` index** lists all registered BUMN with links. *(**v7.0** — the registry holds the full **36-BUMN** portfolio; each new BUMN automatically gets its `/bumn/‹slug›` + `/bumn-v2/‹slug›` dashboards, a `‹slug›@nexorus.io`/`‹slug›2026` scoped login, an allowlist entry, and a CEO-wall board row — adding a BUMN remains one registry row.)*
...
**v7.0 — files add/change (roster 7 → 36):**
- `change` `lib/bumn/registry.ts` — **29 new `BUMN_REGISTRY` rows** (slug = `danantara_‹slug›` minus prefix; `name`/`short`/`sector` per the v7.0 roster). No helper/interface change — `getBumn`/`listBumn`/`isAllowedTopicCode` already iterate the array; the auth logins, `/bumn` index, allowlist, and CEO-wall board all derive automatically.
- `change` `lib/bumn/registry.test.ts` — roster assertion updated to the 36 slugs (the `topicCode === danantara_‹slug›` invariant validates each slug).
- `change` `app/api/v1/danantara/bumn-board/route.ts` *(A7 v37 board)* — **cap the parallel upstream fan-out** (small concurrency pool, e.g. 8) so 36 codes on a cold cache don't hammer/rate-limit the upstream; total upstream calls and output rows unchanged. *(Recorded as an A7 revision — internal perf, no AC change.)*
...
| 7.0 | 2026-06-14 | **MAJOR** — client expanded the listed portfolio from **7 → 36 BUMN** (full topic-code list supplied, all verified live: HTTP 200, 34 with data, 2 temporarily empty). 29 new `BUMN_REGISTRY` rows (name/short/sector derived; sectors mapped onto the existing 7-key taxonomy; new BUMN use the monogram logo fallback). Registry-driven, so dashboards/logins/index/allowlist/board all follow automatically. CEO-wall board fan-out gains a **concurrency cap** (A7 board, internal perf, no AC change). AC1 amended; T1/T2 generalised + T19 added. Status → Built (TDD): registry roster + uniqueness + cap tests; full suite **245 green**, tsc + lint clean (`BumnIndex` test made registry-count-agnostic + O(1)-query to stay fast at 36 rows) |
