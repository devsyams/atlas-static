...
- **Version:** 5.0 · **Stage:** 3-act · **Sprint:** demo · **Status:** Built · **Spec ref:** — · **Owner:** Dev A
...
**Data-model / API changes:** one new BFF route (`POST /api/v1/jasamarga-ai`). *(v2.0)* `/api/v1/jasamarga-ops` now also reads the topics feed server-side; `SocialPulse` gains additive optional fields. No DB. No LLM schema change — the model still fills the **existing** `OpsInsight` and `Prediction` types.
