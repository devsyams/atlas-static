...
- **Version:** 3.0 · **Stage:** 3-act · **Sprint:** demo · **Status:** In progress · **Spec ref:** `docs/superpowers/specs/2026-08-01-board-level-threat-index-response-simulator.md` · **Owner:** Dev A
...
A14 puts it on the page. The **top 3 negative topics** stay as topic cards, but the old share-of-voice bar is replaced by a **board-level Threat Index response simulator**: the overall `crisisIndex` drives the multiplier, **all negative topics** provide the volume anchor, tier selection stays as an intensity control, and the output shows how many KOL posts, clipper captions, and grassroots actions the board should deploy — plus the modeled post-response Threat Index. The client still gets the demo win, but the response story now matches the executive question the room actually asks.
...
- **AC4** — *Given* live AI, *When* the section loads, *Then* each of the 3 topics carries a one-sentence **hostile framing**, a one-sentence **counter angle**, and **exactly 3 Indonesian drafts** (kol ≤280 · clipper ≤150 · grassroots ≤200 chars) with 2–4 `#` hashtags, every claim grounded in that topic's own title / `aiLine` / figures — no fabricated statistics, quotes, officials, dates or policy promises.
...
- **AC8** — *Given* a rendered draft, *When* its copy button is pressed, *Then* `body` + a blank line + the hashtags is written to the clipboard, ready to paste — no placeholders, no wrapping quotes. A per-topic WhatsApp dispatch sends the same content as a brief.
- **AC9** *(v3.0)* — *Given* the board simulator, *When* it renders, *Then* it shows **three tier cards** (no shared headline). Each card shows the tier name (colour-coded), its noise multiplier (`×N NOISE`), a strategy tagline, the **counter-actions** total, the **Clipper / Homeless / KOL** split, and **its own Threat Index projection** (`now → modeled post-response` at *that* tier); **Professional** carries a `DEFAULT` badge. The prior single-tier metric grid (recommended volume, projected reach, negative-volume baseline, grassroots) and the shared headline are **removed**.
