...
- **Version:** 1.0 · **Stage:** 3-act · **Sprint:** demo · **Status:** Built · **Spec ref:** — · **Owner:** Dev A

#### PM
**Background (why):** `/danantara/command` answers two of the three questions an executive asks. The gate answers *how bad is it*; the issue board answers *what is being said*. Nothing on the page answers the one the boss actually asks in the room — **"so what do we post to counter it?"** Today that answer exists only as A9's Communication Response Calculator, which is buried: you must click a negative topic card and scroll the detail modal to find it, and when you do it stops at post **counts** — it never says what those posts should *say*. So the comms team leaves the review with a number and no copy, and the demo's strongest claim ("we don't just watch, we respond") is invisible on the main screen.

A14 puts it on the page. For the **top 3 negative topics by negative reach**, an LLM authors the counter-narrative and **three ready-to-post Indonesian drafts** (KOL · clipper · grassroots), and new **reach-based** math sizes how many posts it takes to close the reach gap — visualised as a share-of-voice bar. The client flagged this as the demo's wow factor: a board member can read the attack, read our answer, see the volume it takes to win, and copy a real post to their phone, without leaving the page.
...
- **AC2** — *Given* a topic and a tier, *When* the plan is computed, *Then* the reach math is pure, deterministic and internally consistent: `hostileReach = round(reach × negShare)`, `totalPosts = ceil(hostileReach × multiplier / blendedReachPerPost)`, the three channel counts **sum exactly to `totalPosts`** (largest-remainder, never naive rounding — the room adds them up), and `shareOfVoicePct + hostileSharePct === 100`.
- **AC3** — *Given* the rendered section, *When* the tier is changed (Basic ×1 → parity, Professional ×3 → 75% SOV, Enterprise ×5 → ~83%), *Then* the post counts, projected reach and share-of-voice recompute **instantly and client-side with no network request**, and the AI-authored content does not change — tier scales volume only.
...
- **AC9** — *Given* the plan, *When* the share-of-voice bar renders, *Then* its hostile and counter segments are sized to `hostileSharePct` / `shareOfVoicePct` and sum to 100%, with the headline % counting up on reveal.
...
#### Architecture
**Impact — files add/change:**
- `add` `lib/danantara/ceo/counter-narrative.ts` (+ `.test.ts`) — the pure reach model. **Reuses** `ResponseTier`/`TIER_MULTIPLIER`/`TIER_LABEL`/`CHANNEL_SHARE` from A9's `counter-noise.ts` (one source of truth for the tier ladder and the 50/20/30 mix); adds `REACH_PER_POST` (`kol 45_000 · clipper 12_000 · homeless 1_200` → blended **19,740**), `CHANNEL_LABEL` (`homeless` → "Grassroots" — the key stays, only the display vocabulary is new, so A9's three signed-off test files are untouched), `hostileReachOf`, `splitPosts` (largest-remainder), `counterNarrativePlan`, `topNegativeByReach`, `aggregateWarRoom`. Does **not** reuse `IMPRESSIONS_PER_POST`/`responseCalculator` — A9 is impressions-based, A14 is reach-based, and both keep rendering.
- `add` `lib/danantara/ceo/counter-narrative-ai.ts` (+ `.test.ts`) — pure prompt/schema/grounding/parse/fallback, mirroring `lib/jasamarga/ai-insight.ts` (A12).
- `add` `app/api/v1/danantara/counter-narrative/route.ts` (+ `.test.ts`) — POST, module-level content-keyed TTL `Map`, always-200 `{source}`, mirroring `app/api/v1/jasamarga-ai/route.ts`.
- `add` `components/danantara/ceo/useCounterNarrative.ts`, `CounterNarrativeWarRoom.tsx` (+ `.test.tsx`), `ShareOfVoiceBar.tsx`, `CounterTopicCard.tsx`, `DraftCard.tsx`.
- `change` `lib/danantara/ceo/response-dispatch.ts` (+ test) — **new export only**, `buildWarRoomBrief`; `buildResponseBrief` untouched so A9 stays green.
- `change` `components/danantara/ceo/DanantaraCommandCenter.tsx` (**A13 v3.0**) — one added sibling.
- `change` `components/danantara/ceo/DanantaraCommandCenter.test.tsx` — T4 request count 4 → 5.

**Data-model / API changes:** one new BFF route; **no DB, no schema, no new upstream feed**. New optional env `DANANTARA_CN_AI_TTL_MIN` (default 360). Reuses `NEXORUS_AI_KEY`/`NEXORUS_AI_MODEL` and `NEXT_PUBLIC_RESPONSE_WHATSAPP`.

**Reuse:** `lib/ai/engine.liveJson` (model-agnostic, token-logged), `lib/ai-settings` kill switch, `engine.groupIssuesBySentiment`, `format.fmtCount`, `SentimentBreakdown.TONE`, `counter-noise` tiers + channel mix, `response-dispatch.whatsappResponseLink`, and the **existing CSS vocabulary with no new `@keyframes`** — including `.cn-bar` and `.cn-blip`, which were written for A9 and have sat unused; `.cn-bar`'s `cn-fill` keyframe is exactly the share-of-voice grow-in.

**Risks:** (1) *Latency* — a 3-topic structured call is ~6–15 s; mitigated in layers by a mount-time fetch while the section is below the fold, an instant deterministic fallback, the honest terminal, a 12 s hard ceiling and in-place upgrade — never a spinner, never a blank. (2) *Cost* — the content-keyed 6 h cache means the topic set changes at most every 6 h, so ~4 calls/day for the whole deployment, not per viewer; `?fresh=1` is the only leak, hence the 60 s floor. (3) *No cache warming* — the scheduled pre-warm was built and dropped (needs Vercel Pro; index row 1.86), so the first load after a topic-set change pays full latency. **Operational note: load `/danantara/command` once ~5 minutes before a demo to warm it.** (4) *Data honesty* — `negShare` derives from **impressions** (`mentions`) and the feed carries no per-topic `negReach`, so "negative reach" is an **estimate**; the on-screen footnote must say so or it will be quoted as measured fact. (5) *`REACH_PER_POST` is a heuristic* — it is the first number an exec challenges; it lives in one constant and real per-channel media data drops straight in. (6) *Model refusal* — mitigated by the disclosed-amplification framing above, and smoke-tested against the live model before the UI is built.
...
| T6 | AC2 | `shareOfVoicePct + hostileSharePct === 100`; basic → 50, professional → 75, enterprise → 83 (±1) | pure |
| T7 | AC3 | one topic at three tiers → identical `hostileReach`, strictly increasing posts/projected reach/SOV | pure |
| T8 | AC2 | `aggregateWarRoom` sums the three plans and recomputes SOV from the totals, not an average | pure |
...
| T26 | AC3 | clicking `war-tier-enterprise` changes the counts and the SOV with **no new fetch** (mock call count unchanged) | component |
| T27 | AC9 | the SOV bar's two segment widths equal `hostileSharePct` / `shareOfVoicePct` and sum to 100% | component |
