...
A14 puts it on the page. The **top 3 negative topics** stay as topic cards, but the old share-of-voice bar is replaced by a **board-level Threat Index response simulator**: the overall `crisisIndex` drives the response posture, **all negative topics** provide the volume anchor, tier selection stays as an intensity control, and the output shows how many KOL posts, clipper captions, and grassroots actions the board should deploy — plus the modeled post-response Threat Index and the longer counter narrative that explains why. The client still gets the demo win, but the response story now matches the executive question the room actually asks.

**Framing constraint (not optional).** The output is coordinated, ready-to-post political content. Framed carelessly it collides with platform rules on coordinated inauthentic behaviour, with Indonesian paid-endorsement disclosure norms, and — most immediately for this architecture — with **model refusal**, which degrades *silently* to the deterministic fallback mid-demo. So the model still generates the internal draft set, but the visible war-room surface only shows the counter narrative and the per-channel post counts; the PR/media team owns the final copy. The system prompt forbids impersonating neutral citizens or independent media, fabricating statistics, and naming or baiting individuals. This is a build requirement, not a disclaimer.
...
- **AC3** — *Given* the rendered section, *When* the tier is changed (Basic ×1, Professional ×3, Enterprise ×5), *Then* the deploy counts, projected reach and modeled post-response Threat Index recompute **instantly and client-side with no network request**, and the topic cards do not change — tier scales board response intensity only.
...
| T24 | AC7/AC12 | route returns `scripted` → 3 topic cards, 9 non-empty count rows, badge `Simulated` | component |
| T25 | AC4 | route returns `llm` → the model's attack line / counter narrative / channel counts are on screen, badge `● Nexorus AI · LLM` | component |
| T26 | AC3 | clicking `war-tier-enterprise` changes the board-simulator counts and modeled post-response Threat Index with **no new fetch** (mock call count unchanged) | component |
| T27 | AC9 | the board simulator renders the Threat Index, response posture, deploy total and channel split; the old SOV bar is absent | component |
| T28 | AC8 | the dispatch brief carries the attack line, counter narrative and channel counts; it does not include draft bodies or clipboard copy flow | component |
...
| T35 | AC8 | `buildWarRoomBrief` carries the title, hostile reach, tier, the channel split and the counter narrative, under 1 800 chars (the `wa.me?text=` practical cap) | pure |
...
| 2.0 | 2026-08-01 | **MAJOR (behavior/scope)** — the war room now shows the counter narrative as a short paragraph plus the per-channel post counts, while the actual draft bodies stay internal to the model. The visible pane no longer renders draft cards or clipboard-copy actions; the dispatch brief carries the narrative and counts-only handoff instead. Spec ref: `docs/superpowers/specs/2026-08-01-board-level-threat-index-response-simulator.md` |
